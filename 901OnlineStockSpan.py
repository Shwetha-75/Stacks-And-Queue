...
class Solution:
    ...
    def next(self, price: int) -> int:
        ...
        while self.stack and self.stock[self.stack[-1]]<=price:
              self.stack.pop()
        if self.stack:
            self.pge.append(self.stack[-1])
        else:
            self.pge.append(-1)
        self.stock.append(price)
        pge=self.pge[-1]
        self.stack.append(len(self.stock)-1)
        return len(self.stock)-pge-1
class TestApp:
      def testing_case_one(self):
          query_1=["StockSpanner", "next", "next", "next", "next", "next", "next", "next"]
          query_2=[[], [100], [80], [60], [70], [60], [75], [85]]
          result=[None]
          price=Solution()
          for i in range(1,len(query_1)):
              if query_1[i]=='next':
                  result.append(price.next(query_2[i]))
