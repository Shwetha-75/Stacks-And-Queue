...
class Solution:
    def sumSubarrayMins(self, arr: list[int]) -> int:
        sum_value,n=0,len(arr)
        for i in range(n):
            mini_value=arr[i]
            for j in range(i,n):
                mini_value=min(mini_value,arr[j])
                sum_value=(sum_value+mini_value)%(10**9+7)
        return sum_value
    
# Previous Smallest Element, Next Smallest Element
class Solution:
    def sumSubarrayMins(self, arr: list[int]) -> int:
        n=len(arr)
        prev_smallest_element=[-1]*n 
        next_smallest_element=[n]*n
        stack=[]
        ...
        for i in range(n):
            while stack and stack[-1][0]>=arr[i]:
                stack.pop()
            if stack:
                prev_smallest_element[i]=stack[-1][1]
        ...
        for i in range(n-1,-1,-1):
            while stack and stack[-1][0]>arr[i]:
                stack.pop()
            if stack:
                next_smallest_element[i]=stack[-1][1]
            stack.append([arr[i],i])
        sum_value=0
        print(prev_smallest_element)
        print(next_smallest_element)
        for i in range(n):
            sum_value=sum_value+(i-prev_smallest_element[i])*(next_smallest_element[i]-i)*arr[i]
            print(sum_value)
        return sum_value 

Solution().sumSubarrayMins([1,4,6,7,3,7,8,1])
